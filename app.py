...
WEBP_FOLDER = 'static/img/webp'
os.makedirs(UPLOAD_FOLDER, exist_ok=True)
os.makedirs(WEBP_FOLDER, exist_ok=True)

# ✅ Compresión y redimensionado optimizado
def convertir_y_comprimir(imagen, destino, calidad=65, size=(300, 300)):
    try:
...
def redimensionar_webp_en_static():
    for nombre in os.listdir(WEBP_FOLDER):
        if nombre.endswith('.webp'):
            ruta = os.path.join(WEBP_FOLDER, nombre)
            try:
                img = Image.open(ruta)
                img = img.convert('RGB')
...
def step2():
    ...
    imagenes = os.listdir(WEBP_FOLDER)
...
def step3():
    ...
    if request.method == 'POST':
        ...
        for i in range(len(nombres)):
            ...
            destino = os.path.join(UPLOAD_FOLDER, webp_name)
